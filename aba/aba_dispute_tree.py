...
class ABA_Dispute_Tree():
    ...
    def __propagate_tree_proponent(self, node):
        ...
        """
        for assumption, symbol in node.assumptions[self.index].items():
    ...
    def __is_infinity(self, node, label):
        ...
        if value:
            logging.debug("Infinity detected in node <%s> of <%s, %s>", node.root, label)
    ...
    def __add_label(self, node, label):
        ...
        if len(node.assumptions[self.index]) > 0:
            self.graph.node[node]['text_label'] += "\nwith assumption(s): %s" % (", ".join(node.assumptions[self.index]))
